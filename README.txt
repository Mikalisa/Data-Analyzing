...
The first step is to spider the repository of url(Note: i've included urls for testing).
...
The program scans the content.sqlite from 1 up to the first message number not
...
Domain names maybe truncated to two levels for .com, .org, .edu, and .net 
other domain names maybe truncated to three levels.  for example si.um.edu becomes
um.edu and caret.cam.ac.uk becomes cam.ac.uk.   Also mail addresses are
...
And all the mail messages will be collected under one sender even if 
...
#a simple vizualization of the word frequence in the subject lines
